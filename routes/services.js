...
router.get('/', auth, async (req, res) => {
  try {
    ...
    const services = await Service.find({ business: business._id })
      .populate('staffMembers', 'name email role googleCalendarIntegrationStatus')
      .sort({ name: 1 });
...
router.post('/', auth, async (req, res) => {
  try {
    const { name, description, duration, price, currency, category, color, maxBookingsPerSlot, staffMembers } = req.body;
    ...
    if (staffMembers && staffMembers.length > 0) {
      ...
      if (validStaff.length !== staffMembers.length) {
        return res.status(400).json({ message: 'One or more staff members are invalid' });
      }
    }
    ...
    const service = new Service({
      ...
      color: color?.trim(),
      maxBookingsPerSlot: maxBookingsPerSlot !== undefined ? parseInt(maxBookingsPerSlot) : 1,
...
router.put('/:serviceId', auth, async (req, res) => {
  try {
    const { name, description, duration, price, currency, category, color, maxBookingsPerSlot, isActive } = req.body;
    ...
    if (price !== undefined) service.price = price !== null ? parseFloat(price) : undefined;
    if (currency !== undefined) service.currency = currency?.toUpperCase() || 'USD';
    if (category !== undefined) service.category = category?.trim();
    if (color !== undefined) service.color = color?.trim();
    if (maxBookingsPerSlot !== undefined) service.maxBookingsPerSlot = parseInt(maxBookingsPerSlot);
...
router.get('/:serviceId/availability', auth, async (req, res) => {
  try {
    ...
    const staffIds = service.staffMembers
      .filter(staff => staff.isActive && staff.googleCalendarIntegrationStatus === 'connected')
      .map(staff => staff._id);
