...
router.put('/:staffId', auth, async (req, res) => {
  try {
    ...
    if (email !== undefined) {
      ...
      if (existingStaff) {
        return res.status(400).json({ message: 'Another staff member with this email already exists' });
      }
...
router.delete('/:staffId', auth, async (req, res) => {
  try {
    ...
    res.status(500).json({ message: 'Error deactivating staff member', error: error.message });
  }
});
...
router.post('/:staffId/generate-calendar-link', auth, async (req, res) => {
  try {
    ...
    res.status(500).json({ message: 'Error generating calendar link', error: error.message });
  }
});
...
router.get('/:staffId/calendar-status', auth, async (req, res) => {
  try {
    ...
    res.status(500).json({ message: 'Error checking calendar status', error: error.message });
  }
});
...
router.put('/:staffId/services', auth, async (req, res) => {
  try {
    ...
    res.status(500).json({ message: 'Error assigning services', error: error.message });
  }
});
