...
timeSlotSchema.methods.isTimeSlotAvailable = function (
	date,
	startTime,
	endTime,
) {
	...
	for (let override of this.dateOverrides) {
		if (override.date.toISOString().split("T")[0] === dateString) {
			...
			if (override.customSlots && override.customSlots.length > 0) {
				return this.checkTimeAgainstSlots(
					startTime,
					endTime,
...
timeSlotSchema.methods.checkTimeAgainstSlots = function (
	startTime,
	endTime,
	slots,
) {
...
timeSlotSchema.statics.getAvailabilityForDateRange = function (
	businessId,
	startDate,
	endDate,
) {
